...
public class ServiceExceptionResponse extends ErrorResponse {
     ...
     */
    @Deprecated
    public ServiceExceptionResponse(String serviceName, Throwable exception, Locale locale) {
    	String code = (String)ReflectionUtility.getFieldValue(exception, "code");
    	MainError mainError = null;
    	SubError subError = null;
    	ArrayList<SubError> subErrors = new ArrayList<SubError>();
    	if(code.equals(SERVICE_CURRENTLY_UNAVAILABLE)) {
    		...
    		String key = (String)ReflectionUtility.invokeGetterMethod(exception, "key");
        	Object[] params = (Object[])ReflectionUtility.getFieldValue(exception, "params");
    		mainError = MainErrors.getError(MainErrorType.BUSINESS_LOGIC_ERROR, locale);
    		serviceName = transform(serviceName);
    		String subErrorCode = ISV + serviceName + SERVICE_ERROR + code;
    		try {
				subError = SubErrors.getSubError(subErrorCode, key, locale, params);
			} catch (Exception e) {
				String message = (String) ReflectionUtility.getFieldValue(exception, "message");
				subError = SubErrors.getSubError(subErrorCode, message);
			}
     ...
     *                    this is a big error
     */
    public ServiceExceptionResponse(RopRequestContext context, Throwable exception) {
    	String code = (String)ReflectionUtility.getFieldValue(exception, "code");	
		...
		if (code.equals(SERVICE_CURRENTLY_UNAVAILABLE)) {
			...
			String subErrorCode = ISV + serviceName + SERVICE_ERROR + code;
			try {
				subError = SubErrors.getSubError(subErrorCode, key, context.getLocale(), params);
			} catch (Exception e) {
				String message = (String) ReflectionUtility.getFieldValue(exception, "message");
				subError = SubErrors.getSubError(subErrorCode, message);
			}
